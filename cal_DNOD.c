		...
		for(size_t j=0; j< ym_size; ++j){
			for(size_t i=0; i< xm_size+1; ++i){
				...
				for(size_t g=0; g<eg_size; ++g){
					...
					double DFDM_val = DFDM->xdata[k][j][i];
					double Jn_val = Jn->xdata[k][j][i];
					DNOD->xdata[k][j][i] = -(DFDM_val * (rval - lval) + Jn_val) / (rval + lval);
				}
			}
		}
	}
		...
		for(size_t k=0; k< zm_size; ++k){
			for(size_t j=0; j< ym_size+1; ++j){
				...
				for(size_t g=0; g<eg_size; ++g){
					...
					double DFDM_val = DFDM->ydata[i][k][j];
					double Jn_val = Jn->ydata[i][k][j];
					DNOD->ydata[i][k][j] = -(DFDM_val * (rval - lval) + Jn_val) / (rval + lval);
				}
			}
		}
	}
		...
		for(size_t i=0; i< xm_size; ++i){
			for(size_t k=0; k< zm_size+1; ++k){
				...
				for(size_t g=0; g<eg_size; ++g){
					...
					double DFDM_val = DFDM->zdata[j][i][k];
					double Jn_val = Jn->zdata[j][i][k];
					DNOD->zdata[j][i][k] = -(DFDM_val * (rval - lval) + Jn_val) / (rval + lval);
				}
			}
		}
	}
